...
<img width="1870" height="1134" alt="image" src="https://github.com/user-attachments/assets/f2b4cda7-f97c-46bf-8c64-7c762ebd2cf0" />
...
=docker compose เป็นเครื่องมือกำหนด และจัดการหลาย container ได้ง่ายในไฟล์เดียว  มีความสำคัญถ้าไม่มี Docker Compose คุณจะต้องสั่ง docker run ทีละตัว พร้อมตั้งค่า port, network, volume เองหมด

2. GitHub pipeline คืออะไร เกี่ยวข้องกับ CI/CD อย่างไร
คือเครื่องมือของ GitHub ที่ช่วยให้คุณสั่งให้ GitHub ทำงานบางอย่างให้อัตโนมัติ ทุกครั้งที่มีการ push, pull request หรือ action อื่น ๆ   
 CI (Integration) ทดสอบทุกครั้งที่ push  รัน `pytest`, `build`, `lint` อัตโนมัติ 
 CD (Deployment)   ส่งโค้ดขึ้นเซิร์ฟเวอร์     Deploy ไปยัง Heroku, Docker, AWS

3. จากไฟล์ docker compose  ส่วนของ volumes networks และ healthcheck มีความสำคัญอย่างไร
 volumes คือที่เก็บข้อมูลถาวรของ container ปกติถ้า container ถูกลบ ข้อมูลก็จะหาย
แต่ถ้าใช้ volumes ข้อมูลจะ “อยู่ข้างนอก” container และไม่หายแม้จะลบ container ไป
 Docker network ทำให้ container หลายตัว เชื่อมกันได้ เหมือนอยู่ใน LAN วงเดียวกัน ถ้าไม่กำหนด network เอง Docker จะสร้างให้แบบสุ่มและแยก container ออกจากกัน
...
jobs:
 ...
 🔹 กำหนดว่าเราจะสร้าง งานทดสอบ (test job) ชื่อว่า Run Tests
 🔹 เป็นส่วนหนึ่งของระบบ GitHub Actions Pipeline ที่จะทำงานอัตโนมัติทุกครั้งที่มีการ push หรือ PR

    runs-on: ubuntu-latest

🔹 GitHub จะใช้ เครื่องเสมือน (runner) ที่ติดตั้ง Ubuntu เวอร์ชันล่าสุดมารันคำสั่งใน job นี้
🔹 เปรียบเหมือนเครื่องคอมที่เราให้ GitHub ใช้รันคำสั่งแทนเรา

    services:
      postgres:
        image: postgres:16-alpine

🔹 กำหนดว่าเราจะให้ GitHub รัน container ของ PostgreSQL ขึ้นมาพร้อม ๆ กับเครื่อง Ubuntu
🔹 ใช้ image postgres:16-alpine ซึ่งเล็กและเร็ว เหมาะกับการทดสอบ
...
🔹 คือการบอกว่าฐานข้อมูลจะมี:
                          ผู้ใช้ชื่อ testuser
                          รหัสผ่าน testpass
                          ชื่อฐานข้อมูล testdb
        ports:
          - 5432:5432

🔹 เปิดพอร์ต 5432 ของ PostgreSQL (ซึ่งเป็น default port)
🔹 เพื่อให้เครื่องที่ใช้รันทดสอบสามารถเชื่อมเข้า PostgreSQL ได้
...
🔹 เป็นการตั้งค่า health check สำหรับ PostgreSQL container
🔹 เพื่อให้แน่ใจว่า database พร้อมทำงานจริงก่อนที่ job จะรันคำสั่งอื่น
```
...
uses: actions/checkout@v4	ดึงโค้ดจาก GitHub repo มาบนเครื่อง	GitHub ต้องมีโค้ดถึงจะรันเทสได้
uses: actions/setup-python@v5	ติดตั้ง Python เวอร์ชันที่ต้องการ	เพื่อให้รัน pip install, pytest, หรือโค้ด Python ได้
```
6. Snyk คืออะไร มีความสามารถอย่างไรบ้าง
   คือเครื่องมือ ตรวจหาช่องโหว่ด้านความปลอดภัยในโค้ดและ dependency แบบอัตโนมัติ
ใช้ได้กับหลายภาษา เช่น Python, Node.js, Java รวมถึง Docker และ Kubernetes

ความสามารถหลัก	
 SCA (Software Composition Analysis)	ตรวจสอบว่า dependency (เช่น Flask, psycopg2) มีช่องโหว่หรือไม่
 Code Scanning	วิเคราะห์ source code หาช่องโหว่ (เหมือน static code analysis)
 Docker Image Scanning	สแกน container image ว่ามี package หรือ config ที่เสี่ยงหรือไม่
 CI/CD Integration	ใช้ใน GitHub Actions, GitLab CI, Jenkins ได้เพื่อบล็อก code ที่ไม่ปลอดภัย
 Fix Suggestions	ช่วยแนะนำวิธีอัปเกรด dependency หรือแก้ช่องโหว่ให้อัตโนมัติ
 License Check	ตรวจสอบว่า package ต่าง ๆ ใช้ license ที่ถูกต้องหรือไม่
