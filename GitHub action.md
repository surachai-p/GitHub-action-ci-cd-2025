...
<img width="1915" height="1079" alt="image" src="https://github.com/user-attachments/assets/f0a7d1d1-d683-4a27-8175-eade0488f39b" />
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/23791249-bfae-472d-957e-331c778cb884" />
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/35143167-83e1-4ace-8da4-da1ffaa5aa15" />
...
<img width="1912" height="1079" alt="image" src="https://github.com/user-attachments/assets/ec0f73d3-8106-4aea-9c8c-00cb50d4f42c" />
...
```
## บันทึกรูปผลการทดลอง 
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/b54cea60-44f9-4900-aa16-438ff0ac80b7" />
...
#### ตอบ :
  - Docker Compose คือเครื่องมือที่ช่วยกำหนดและรันแอปพลิเคชันที่มีหลายคอนเทนเนอร์พร้อมกัน (เช่น เว็บแอป + ฐานข้อมูล) ได้อย่างง่ายดายด้วยคำสั่งเดียว โดยการตั้งค่าทั้งหมดจะถูกเขียนไว้ในไฟล์ YAML
  - มีความสำคัญในการจัดการระบบซับซ้อน ทำให้การจัดการหลายบริการเป็นระบบและง่ายขึ้น และสร้างสภาพแวดล้อมซ้ำได้ ทุกคนในทีมสามารถรันสภาพแวดล้อมการพัฒนาเดียวกันได้อย่างรวดเร็ว

2. GitHub pipeline คืออะไร เกี่ยวข้องกับ CI/CD อย่างไร
#### ตอบ : 
- GitHub Pipeline คือ เวิร์กโฟลว์อัตโนมัติ (Automated Workflow) ที่ใช้ GitHub Actions ในการสร้าง, ทดสอบ, และปรับใช้โค้ดที่เก็บอยู่ใน GitHub
 - มีความเกี่ยวข้องกับ CI/CD: 
- CI (Continuous Integration): Pipeline จะ สร้างและทดสอบ โค้ดใหม่ทันทีที่ถูก Push หรือเปิด Pull Request เพื่อหาข้อผิดพลาดตั้งแต่เนิ่น ๆ
- CD (Continuous Delivery/Deployment): หลังจากทดสอบผ่าน Pipeline จะดำเนินการ ปรับใช้ (Deploy) แอปพลิเคชันไปยังเซิร์ฟเวอร์โดยอัตโนมัติหรือกึ่งอัตโนมัติ

3. จากไฟล์ docker compose  ส่วนของ volumes networks และ healthcheck มีความสำคัญอย่างไร
 #### ตอบ : 
 - volumes ความสำคัญ: ทำให้ข้อมูลถาวร (Data Persistence) ไม่หายไปเมื่อคอนเทนเนอร์ถูกลบหรือสร้างใหม่ และใช้สำหรับ แชร์โค้ด/ไฟล์ ระหว่างเครื่องโฮสต์กับคอนเทนเนอร์ (Bind Mounts)
 - networks ความสำคัญ ทำให้คอนเทนเนอร์สื่อสารกันได้ ด้วยชื่อบริการ (Service Name) โดย Compose จะสร้างเครือข่ายเสมือนให้คอนเทนเนอร์ทำงานร่วมกันได้
 - healthcheck ความสำคัญ ตรวจสอบว่าแอปพลิเคชันภายในคอนเทนเนอร์นั้น พร้อมใช้งาน (Ready) จริง ๆ ไม่ใช่แค่ "รันอยู่" เพื่อให้มั่นใจว่าบริการอื่นจะไม่เริ่มทำงานจนกว่าบริการที่พึ่งพาจะพร้อม 100%
...
```
#### ตอบ :
โค้ดนี้คือการตั้งค่างานใน Pipeline ของ GitHub Actions เพื่อรันการทดสอบ โดยมีขั้นตอนหลักดังนี้:
- runs-on: ubuntu-latest	กำหนดให้รันงานนี้บนเครื่องเสมือน Ubuntu (Runner)
- services: postgres:	สร้างคอนเทนเนอร์ฐานข้อมูล PostgreSQL (ใช้ Image postgres:16-alpine) เพื่อใช้ในการทดสอบ
- env:	ตั้งค่าชื่อผู้ใช้ (testuser), รหัสผ่าน (testpass) และชื่อฐานข้อมูล (testdb) ที่จำเป็นสำหรับฐานข้อมูล
- ports: - 5432:5432	แมปพอร์ตมาตรฐานของ PostgreSQL (5432) เพื่อให้โค้ดทดสอบเชื่อมต่อได้
- options: >-	กำหนดการตรวจสอบสุขภาพ (Health Check) ของฐานข้อมูล
- ```--health-cmd "pg_isready..."```	ใช้คำสั่งเช็กว่าฐานข้อมูล พร้อมรับการเชื่อมต่อแล้วจริง ๆ
- ```--health-interval 10s	``` ตรวจสอบสุขภาพ ทุก ๆ 10 วินาที

- เป้าหมายหลัก: ทำให้มั่นใจว่า ฐานข้อมูล PostgreSQL พร้อมทำงานอย่างสมบูรณ์ ก่อนที่ Job จะเริ่มรันโค้ดทดสอบจริง เพื่อป้องกันการล้มเหลวจากการเชื่อมต่อฐานข้อมูลไม่ทัน
...
```
#### ตอบ :
- โค้ดนี้อยู่ในส่วนของ steps: ใน Job ของ GitHub Actions ซึ่งเป็นการกำหนดขั้นตอนที่ Job จะต้องทำ โดย uses: เป็นคำสั่งสำคัญที่หมายถึงการเรียกใช้ Action ที่ถูกสร้างไว้ล่วงหน้า (Pre-built Action)
1. uses: actions/checkout@v4 (Checkout code)
Action นี้ทำหน้าที่ ดึงโค้ด (Source Code) จาก GitHub repository ของคุณมาไว้ในเครื่อง Runner (Ubuntu) เพื่อให้ Job สามารถเข้าถึงไฟล์ต่าง ๆ และรันการทดสอบหรือ Build แอปพลิเคชันได้
  -  ```uses``` :	บอกว่าจะใช้ Action ที่ชื่อว่า actions/checkout
  -  ```@v4```:	ระบุว่าจะใช้ Action เวอร์ชัน 4 (ซึ่งเป็นเวอร์ชันที่เสถียรและแนะนำ)

สรุป: เป็นขั้นตอนบังคับขั้นตอนแรกสุด เพื่อ นำโค้ดของคุณเข้ามาใน Pipeline

2. uses: actions/setup-python@v5 (Set up Python)
Action นี้ทำหน้าที่ ติดตั้งและตั้งค่าสภาพแวดล้อม Python ที่จำเป็นสำหรับการรันโค้ดของคุณในเครื่อง Runner


  - ```uses``` :	บอกว่าจะใช้ Action ที่ชื่อว่า actions/setup-python
  - ```@v5```	ระบุว่าจะใช้ Action เวอร์ชัน 5
  - ```python-version```:	กำหนดว่าต้องการติดตั้ง Python เวอร์ชันใด (ดึงค่ามาจาก Environment Variable ที่ชื่อ PYTHON_VERSION)
  - ```cache: 'pip'```	เปิดใช้งานการ แคช (Caching) สำหรับแพ็กเกจของ Python ที่จัดการด้วย pip (เช่น ไฟล์ในโฟลเดอร์ .pip-cache) ทำให้การติดตั้ง Dependencies ในรอบถัด ๆ ไป รวดเร็วขึ้นมาก

6. Snyk คืออะไร มีความสามารถอย่างไรบ้าง
#### ตอบ :
   - Snyk คือแพลตฟอร์ม DevSecOps ที่ช่วยให้สามารถค้นหาและแก้ไขช่องโหว่ด้านความปลอดภัยในโค้ด, dependencies, containers, และ infrastructure code (IaC) ได้ตั้งแต่เนิ่น ๆ ตลอดวงจรการพัฒนาซอฟต์แวร์
   
  - ความสามารถหลักของ Snyk
- Snyk Open Source : สแกนหาช่องโหว่ ตรวจสอบ Library และ Dependencies ที่คุณใช้ในโปรเจกต์ (เช่น npm, Maven, pip) เพื่อค้นหาช่องโหว่ความปลอดภัยที่ถูกเปิดเผยแล้ว
- Snyk Code : สแกนโค้ดของคุณโดยตรง ตรวจสอบโค้ดที่คุณเขียนขึ้นเอง (Source Code) เพื่อหาช่องโหว่ทั่วไป เช่น SQL Injection, Cross-Site Scripting (XSS), หรือการจัดการข้อมูลที่ไม่ปลอดภัย
- Snyk Container : สแกน Docker Images ตรวจสอบ Base Image, Layer ต่าง ๆ, และ Libraries ภายใน Docker Container เพื่อหาช่องโหว่ด้านความปลอดภัยและปัญหาด้านการตั้งค่า (Configuration)
- Snyk Infrastructure as Code (IaC) : ตรวจสอบไฟล์การตั้งค่า สแกนไฟล์ Infrastructure Code (เช่น Terraform, CloudFormation, Kubernetes YAML) เพื่อหาการตั้งค่าที่ผิดพลาดซึ่งอาจทำให้เกิดช่องโหว่ด้านความปลอดภัยในระบบคลาวด์ (Misconfigurations)
