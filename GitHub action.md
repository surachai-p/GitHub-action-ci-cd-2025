...
```
<img width="1710" height="1112" alt="ภาพถ่ายหน้าจอ 2568-10-02 เวลา 10 57 52" src="https://github.com/user-attachments/assets/7edd6cff-4222-4ff4-a725-d39bd2992e17" />
...
```
ตอบคำถาม
    1.Docker compose คือเครื่องมือที่ใช้ในการจัดการ container หลายตัวให้ทำงานร่วมกันได้อย่างสะดวกโดยอาศัยไฟล์กำหนดค่าเพียงไฟล์เดียว นักพัฒนาสามารถกำหนด service ต่างๆเช่น web database หรือ cache และสั่งให้ทำงานพร้อมกันได้ การใช้งาน docker compose ทำให้การพัฒนาและทดสอบระบบเป็นไปอย่างรวดเร็วและมีความสม่ำเสมอทั้งในเครื่องของผู้พัฒนาและเซิร์ฟเวอร์จริง

    2.GitHub pipeline คือกระบวนการทำงานอัตโนมัติที่กำหนดบน GitHub Actions ใช้ในการจัดการขั้นตอนการทำงาน เช่น การ build การทดสอบ และการ deploy ทุกครั้งที่มีการเปลี่ยนแปลงโค้ด ซึ่ง pipeline มีความเกี่ยวข้องโดยตรงกับแนวคิด CI และ CD โดย CI หรือ Continuous Integration จะทำการทดสอบและตรวจสอบโค้ดใหม่ทุกครั้งที่มีการ commit เพื่อป้องกันไม่ให้โค้ดเสียหาย ส่วน CD หรือ Continuous Delivery และ Continuous Deployment จะช่วยทำให้การส่งโค้ดขึ้นสู่เซิร์ฟเวอร์หรือระบบจริงเกิดขึ้นได้อย่างต่อเนื่องและอัตโนมัติ

    3.ภายในไฟล์ docker compose มีส่วนสำคัญคือ volumes networks และ healthcheck โดย volumes ใช้ในการเก็บข้อมูลถาวร เช่น ข้อมูลฐานข้อมูลที่ไม่ต้องการให้หายไปแม้ container ถูกลบออกไป ส่วน networks ใช้เพื่อสร้างเครือข่ายเชื่อมต่อระหว่าง container หลายตัวภายในระบบเดียวกัน ทำให้ container ติดต่อหากันได้ด้วยชื่อ service และ healthcheck ใช้สำหรับตรวจสอบสถานะความพร้อมของ container ว่าพร้อมใช้งานจริงหรือไม่ก่อนให้ container อื่นเข้ามาเชื่อมต่อ

    4.ในตัวอย่างโค้ด yaml ของ GitHub Actions jobs test จะทำการสร้างงานที่รันบนเครื่อง ubuntu และมีการสร้าง service postgres เพื่อใช้เป็นฐานข้อมูลสำหรับการทดสอบ โดยกำหนด environment เช่นชื่อผู้ใช้ รหัสผ่าน และชื่อฐานข้อมูล รวมถึงเปิดพอร์ต 5432 เพื่อให้ติดต่อได้ พร้อมกับกำหนด healthcheck เพื่อตรวจสอบความพร้อมของฐานข้อมูลก่อนใช้งาน

    5.ส่วนของ uses actions checkout หมายถึงการดึงซอร์สโค้ดจาก repository ลงมายัง runner เพื่อให้ขั้นตอนต่อไปสามารถทำงานกับโค้ดได้ และ uses actions setup python ใช้ในการติดตั้งและตั้งค่าเวอร์ชันของ Python ที่ต้องการใช้งาน รวมถึงสามารถ cache dependency ของ pip เพื่อลดเวลาในการทำงานซ้ำใน pipeline

    6.Snyk เป็นเครื่องมือด้านความปลอดภัยที่ใช้ตรวจสอบช่องโหว่ทั้งในโค้ดและ dependency ของโครงการ รวมถึงสามารถสแกน docker image และไฟล์กำหนดค่าของโครงสร้างพื้นฐาน เช่น Kubernetes หรือ Infrastructure as Code ได้ ความสามารถหลักของ Snyk คือการแจ้งเตือนและแนะนำวิธีแก้ไขเมื่อพบ dependency ที่ไม่ปลอดภัย ช่วยให้นักพัฒนาสามารถป้องกันปัญหาด้านความปลอดภัยตั้งแต่ขั้นตอนการพัฒนาไปจนถึงการนำระบบขึ้นใช้งานจริง
```
