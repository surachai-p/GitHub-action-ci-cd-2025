...
ป็นเครื่องมือที่ใช้จัดการหลาย container พร้อมกันผ่านไฟล์ docker-compose.yml ช่วยให้รันระบบทั้งหมดได้ง่ายและเป็นมาตรฐานเดียวกันทุกเครื่อง
2. GitHub pipeline คืออะไร เกี่ยวข้องกับ CI/CD อย่างไร
คือกระบวนการอัตโนมัติใน GitHub Actions ที่ใช้รันงาน เช่น test, build, deploy ซึ่งเป็นส่วนสำคัญของระบบ CI/CD เพื่อทดสอบและส่งโค้ดอัตโนมัติ
3. จากไฟล์ docker compose  ส่วนของ volumes networks และ healthcheck มีความสำคัญอย่างไร
	...
	•	networks เชื่อม container ให้สื่อสารกันได้
	•	healthcheck ตรวจสอบว่า service พร้อมใช้งานหรือไม่
    ...
    services:
      postgres:
        ...
        options: >-
          ...
          --health-retries 5
```
โค้ดนี้สร้าง job ชื่อ “Run Tests” บน Ubuntu และรัน container PostgreSQL สำหรับใช้ทดสอบ โดยตั้งค่าฐานข้อมูล (user, password, db) และเพิ่ม health check (pg_isready) เพื่อตรวจว่าฐานข้อมูลพร้อมก่อนรันทดสอบ
    ...
    steps:
      ...
      - name: Set up Python
        ...
        with:
          ...
          cache: 'pip'
```
	•	actions/checkout@v4 👉 ใช้ให้ GitHub Actions ดึงโค้ดจาก repo มาไว้ใน runner เพื่อให้ pipeline เข้าถึงไฟล์โค้ดได้
	•	actions/setup-python@v5 👉 ใช้เพื่อติดตั้งและตั้งค่า Python เวอร์ชันที่ต้องการ บน runner ก่อนรันคำสั่งต่าง ๆ (เช่น pip install หรือ pytest)

6. Snyk คืออะไร มีความสามารถอย่างไรบ้าง
Snyk คือเครื่องมือสำหรับ สแกนช่องโหว่ความปลอดภัย ในโค้ด, dependency, Docker image และ Kubernetes ช่วยตรวจหาและแนะนำวิธีแก้ไขปัญหาความปลอดภัยก่อน deploy
