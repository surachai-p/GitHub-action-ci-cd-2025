...
- Docker Compose คือเครื่องมือที่ช่วยจัดการ multi-container applications โดยใช้ไฟล์ docker-compose.yml เพียงไฟล์เดียวในการกำหนดว่า container ใดต้องรัน, ใช้ image อะไร, เปิด port ใด, รวมถึงการจัดการ network และ volume
ความสำคัญ
  -ทำให้การรันหลาย container เช่น Backend + Database + Frontend ง่ายขึ้น
  -ใช้ไฟล์ configuration เดียว รันได้ทุก environment (dev, staging, prod)
  -ลดขั้นตอน manual (ไม่ต้อง docker run หลาย ๆ คำสั่งเอง)
2. GitHub pipeline คืออะไร เกี่ยวข้องกับ CI/CD อย่างไร
  -GitHub Pipeline คือ workflow อัตโนมัติบน GitHub Actions ที่เขียนใน .github/workflows/*.yml ใช้สำหรับ build, test, lint และ deploy code
  -CI: รันทดสอบ/ตรวจโค้ดอัตโนมัติเมื่อมี push หรือ PR เพื่อมั่นใจว่าโค้ดไม่พัง
  -CD: เมื่อ test ผ่าน สามารถ deploy ไป staging/production ได้ทันที

3. จากไฟล์ docker compose  ส่วนของ volumes networks และ healthcheck มีความสำคัญอย่างไร
-volumes: เก็บข้อมูลถาวรของ container และใช้แชร์ไฟล์/config ระหว่างกัน
-networks: เชื่อม container หลายตัวให้สื่อสารกันอย่างปลอดภัย โดยไม่ต้องเปิด port สู่ภายนอก
-healthcheck: ตรวจสอบความพร้อมของ container ก่อนให้ service อื่นใช้งาน เพื่อลดปัญหาการรันแต่ยังไม่พร้อม
    ...
    services:
      postgres:
        ...
        options: >-
          ...
          --health-retries 5
```
  - ไฟล์ YAML สร้าง job ทดสอบ (Run Tests) ที่รันบน Ubuntu พร้อมกับ spin up Postgres service ใช้ทดสอบ โดยมี environment variables, port mapping, และ healthcheck เพื่อให้แน่ใจว่า DB พร้อมใช้งานก่อนจะรันทดสอบ
    ...
    steps:
      ...
      - name: Set up Python
        ...
        with:
          ...
          cache: 'pip'
```
-uses: actions/checkout@v4 → ดึงโค้ดจาก repository มายัง runner (เหมือน git clone) เพื่อให้ขั้นตอนต่อไปเช่น build/test ใช้งานได้

-uses: actions/setup-python@v5 → ติดตั้ง Python เวอร์ชันที่ต้องการบน runner และรองรับการ cache dependency (pip) ทำให้ pipeline ใช้งาน Python และ library ได้ทันที
6. Snyk คืออะไร มีความสามารถอย่างไรบ้าง
  คือเครื่องมือที่ช่วยนักพัฒนาตรวจหาช่องโหว่ใน โค้ด, dependency, container, และ config
 
1.Dependency Scanning – ตรวจหาช่องโหว่ในไลบรารี/แพ็กเกจ
2.Container Scanning – สแกน Docker image หาแพ็กเกจเสี่ยง
3.IaC Scanning – ตรวจ config ของ Terraform/Kubernetes ที่ไม่ปลอดภัย
4.Code Scanning – วิเคราะห์โค้ดหาช่องโหว่โดยตรง
5.CI/CD Integration – ผูกเข้ากับ pipeline เพื่อบล็อกโค้ดไม่ปลอดภัย
